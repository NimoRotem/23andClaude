...
```
nginx :443  ──►  uvicorn :<PORT>   simple-genomics/app.py
...
A second app (`/ancestry/`) proxies to port <PORT> and is out of scope for
this document.
...
```
   ...
   └──────────────────────┘
            ...
            ▼
   ┌──────────────────────┐
   │ Plink2 pgen cache    │  <DATA_ROOT>/pgen_cache/<key>/sample.{pgen,pvar,psam}
...
| ---------------------------------------- | ------------------------------------------ | --- |
| `<CACHE_ROOT>/pgs_cache/<PGS_ID>/`              | downloaded + normalized PGS scoring files  | manual; ingest is idempotent |
| `<CACHE_ROOT>/pgs_cache/pca_1000g/`             | LD-pruned PCA reference cache (one-time)   | manual rebuild |
| `<CACHE_ROOT>/pgs_cache/_all_pgs_pca_positions_{chr,bare}.tsv` | union of PGS+PCA positions | rebuild if any scoring file newer than cache |
| `<DATA_ROOT>/pgen_cache/<key>/sample.{pgen,pvar,psam}` | plink2 pgen built from user's VCF | mtime of source VCF + `PGEN_CACHE_SCHEMA` (currently `v5`) |
| `simple-genomics/cram_vcf_cache/<hash>/` | per-CRAM derived VCFs (PCA-sites, etc.)    | manual; keyed by CRAM realpath sha |
| `<CACHE_ROOT>/ref_stats/` (new) + `<DATA_ROOT>/pgs2/ref_panel_stats/` (legacy) | per-(PGS × pop) reference μ/σ | regenerated by `recompute_ref_stats.py`; loader refuses on schema mismatch |
